%{?scl:%scl_package perl-Package-Stash-XS}
%{!?scl:%global pkg_name %{name}}

Name:		%{?scl_prefix}perl-Package-Stash-XS
Version:	0.28
Release:	5.sc1%{?dist}
...
BuildRequires:	%{?scl_prefix}perl >= 3:5.8.1
BuildRequires:	%{?scl_prefix}perl(base)
BuildRequires:	%{?scl_prefix}perl(constant)
BuildRequires:	%{?scl_prefix}perl(ExtUtils::MakeMaker)
BuildRequires:	%{?scl_prefix}perl(File::Temp)
BuildRequires:	%{?scl_prefix}perl(lib)
%if ( %(perl -e 'print $] >= 5.014 ? 1 : 0;') || 0%{?scl:1} )
...
# No Variable::Magic in EPEL-5 and SCL
%if "%{?rhel}" != "5"
...
%changelog
* Thu Dec 12 2013 Jitka Plesnikova <jplesnik@redhat.com> - 0.28-5
- Rebuilt for newer meta package perl516
- Resolves: rhbz#1040848

* Fri Nov 22 2013 Jitka Plesnikova <jplesnik@redhat.com> - 0.28-4
- Rebuilt for SCL
